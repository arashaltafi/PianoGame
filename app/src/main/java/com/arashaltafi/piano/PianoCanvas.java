...
class PianoCanvas extends SurfaceView implements SurfaceHolder.Callback {
    ...
    public PianoCanvas(Context context, AttributeSet as, int defStyle) {
        ...
        try {
            ...
            Log.e("Piano::DrawingCanvas", "Can't read screen size");
            throw ex;
        }
        ...
        Log.d("Piano::DrawingCanvas", "Display is " + screen_size.x + "x" + screen_size.y +
    ...
    void draw_icon_on_black_key(final Canvas canvas, final Drawable icon, Integer key_idx,
        ...
        r.top = icon_height;
        r.bottom = icon_height + icon_height;
    ...
    void on_key_up(int key_idx) {
        Log.d("Piano::DrawingCanvas", "Key " + key_idx + " is now UP");
        piano.on_key_up(key_idx);
        appConfigHandler.onKeyUp();
        redraw();
    }

    void on_key_down(int key_idx) {
        Log.d("Piano::DrawingCanvas", "Key " + key_idx + " is now DOWN");
        ...
        switch (event.getActionMasked()) {
            ...
            case MotionEvent.ACTION_POINTER_DOWN: {
                if (touch_pointer_to_keys.containsKey(ptr_id)) {
                    Log.e("Piano::DrawingCanvas", "Touch-track error: Repeated touch-down event received");
                    resetPianoState();
            ...
            case MotionEvent.ACTION_MOVE: {
                if (!touch_pointer_to_keys.containsKey(ptr_id)) {
                    Log.e("Piano::DrawingCanvas", "Touch-track error: Missed touch-up event");
                    resetPianoState();
                    return super.onTouchEvent(event);
                }
                ...
                if (touch_pointer_to_keys.get(ptr_id) != key_idx) {
                    Log.d("Piano::DrawingCanvas", "Moved to another key");
            ...
            case MotionEvent.ACTION_UP: {
                if (!touch_pointer_to_keys.containsKey(ptr_id)) {
                    Log.e("Piano::DrawingCanvas", "Touch-track error: Repeated touch-up event received");
                    resetPianoState();
                    return super.onTouchEvent(event);
                }
